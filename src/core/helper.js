...
export const getAvailableGetters = (
  ...
  if (typeof Proxy !== 'undefined') {
    ...
    getters = pluginHost.knownKeys ('Getter').reduce ((acc, getterName) => {
...
export const getAvailableActions = (
  ...
  getAction = actionName =>
    pluginHost.collect (`${actionName}Action`).slice ().reverse ()[0]
) => {
  let actions;
  if (typeof Proxy !== 'undefined') {
    actions = new Proxy (
    ...
    actions = pluginHost.knownKeys ('Action').reduce ((acc, actionName) => {
